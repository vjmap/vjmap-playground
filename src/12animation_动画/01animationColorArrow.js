...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/animation/01animationColorArrow
        // --彩色线箭头动画图层--
        // 地图服务对象
        ...
        polylines.addTo(map);
        polylines.clickLayer(e => message.info(`您点击了第 ${e.features[0].id} 个，名称为 ${e.features[0].properties.name}`))
        polylines.hoverPopup(f => `<h3>ID: ${f.properties.name}`, { anchor: 'bottom' });
        
        
        // 第二次只绘制箭头。背景色设置为透明色。并且没有事件
