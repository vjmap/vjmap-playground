...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/animation/07animationFillImage
        // --填充水滴动画效果[加载本地图片]--
        // 地图服务对象
        ...
        let animateImages = [];
        for(let i = 0; i < 6; i++) {
            let imageId = `water${i + 1}`;
            await map.loadImageEx(imageId, env.assetsPath + `images/${imageId}.png`);
