...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/animation/02animationAntPath
        // --蚂蚁线动画图层--
        // 地图服务对象
        ...
        antPathAnimateLine.polyline.clickLayer(e => message.info(`您点击了第 ${e.features[0].id} 个，名称为 ${e.features[0].properties.name}`))
        antPathAnimateLine.polyline.hoverPopup(f => `<h3>ID: ${f.properties.name}`, { anchor: 'bottom' });
        ...
        const changeArrowAnimateLine = (action) => {
            if (!antPathAnimateLine) return;
            if (action === "stopAnimation") {
                // 停止动画
                antPathAnimateLine.stopAnimation();
            } else if (action === "startAnimation") {
                // 停止动画
                antPathAnimateLine.startAnimation();
            } else if (action === "setSpeedFaster") {
                // 速度变快
                speed *= 2;
                antPathAnimateLine.setSpeed(speed);
                ...
                antPathAnimateLine.updateImages(antPathAnimateLine.createAnimateImages({
                    directionReverse: dirReverse
                }));
            } else if (action === "setColor") {
                // 设置颜色
                antPathAnimateLine.updateImages(antPathAnimateLine.createAnimateImages({
                    fillColor1: vjmap.randInt(0, 2) > 0 ? vjmap.randomColor() : "", // 为空的话，表示是透明，不绘制
                    fillColor2: vjmap.randomColor(),
                }));
                ...
                // 修改坐标
                antPathAnimateLine.updateData(data)
