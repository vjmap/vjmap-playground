...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/animation/animationCircle
        // --动画圆--
        // 地图服务对象
        ...
        const radius1 = mapBounds.width() / 20;
        const radius2 = mapBounds.width() / 4;
        ...
        const mapProgressToValues = vjmap.interpolate(
            [0, 1],
            [
                { r: radius1, color: "#0f0", opacity: 0.8 },
                { r: radius2, color: "#ff0", opacity: 0.2 }
            ]
        )
