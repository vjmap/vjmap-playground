...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/threelayer/04threeLayerRadialGradient
        // --径向渐变球--
        ...
        let radialGradients = []
        const addRadialGradients= () => {
            if (radialGradients.length > 0) return;
            for (let i = 0; i < 50; i++) {
                let pt = mapExtent.randomPoint();
                let mesh = threeContext.radialGradient(pt, {
        ...
        const removeRadialGradients = () => {
            if (radialGradients.length == 0) return;
            for(let c of radialGradients) {
                c.stop();
                    ...
                    <div className="input-card">
                        ...
                        <div>
                            <div className="input-item">
                                <button className="btn" onClick={() => removeRadialGradients()}>移除</button>
                                <button className="btn" onClick={() => addRadialGradients()}>添加</button>
                            </div>
                        </div>
                    </div>
                </div>
