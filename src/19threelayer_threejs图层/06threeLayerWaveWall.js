...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/threelayer/06threeLayerWaveWall
        // --波动光墙--
        ...
        let waveWalls = []
        const addwaveWalls = () => {
            if (waveWalls.length > 0) return;
            let pt = map.fromLngLat(map.getCenter());
            let mesh = threeContext.waveWall(pt);
            waveWalls.push(mesh)
        }
        addwaveWalls();
        ...
        const removewaveWalls = () => {
            if (waveWalls.length == 0) return;
            for (let c of waveWalls) {
                c.stop();
                    ...
                    <div className="input-card">
                        ...
                        <div>
                            <div className="input-item">
                                <button className="btn" onClick={() => removewaveWalls()}>移除</button>
                                <button className="btn" onClick={() => addwaveWalls()}>添加</button>
                            </div>
                        </div>
                    </div>
                </div>
