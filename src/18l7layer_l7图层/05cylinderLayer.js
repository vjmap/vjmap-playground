...
window.onload = async () => {
    ...
    try {
        // 在线效果查看地址: https://vjmap.com/demo/#/demo/map/l7layer/05cylinderLayer
        // --渐变柱状图层-
        // js代码
        ...
        let datas = mapBounds.randomGeoJsonPointCollection(1000);
        
        const pointLayer = new L7.PointLayer({})
            .source(map.toLngLat(datas))
            .shape('cylinder')
            .size('index', idx =>  {
                return [ 1, 1, vjmap.randInt(1, 100) ];
            })
            .animate(true)
            .active(true)
            .color('#006CFF')
        ...
        pointLayer.on('mousemove', e => {
            const popup = new L7.Popup({
                ...
                .setLnglat(e.lngLat)
                .setHTML(`<span>index: ${e.feature.id}</span>`);
